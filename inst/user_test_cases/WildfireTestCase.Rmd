...
```{r}
library(dplyr)
## BA: Directly pulled from http://afsmaps.blm.gov/imf_firehistory/imf.jsp?site=firehistory
fire_dir <- "~/Downloads/Extract_ilmakfs3ws103516312013"
library(maptools)
ak_fire <- readShapeSpatial(paste(fire_dir, "FirePointHistory", sep = "/"))
ak_fire <- ak_fire@data
summary(ak_fire$Latitude)
summary(ak_fire$Longitude)

library(ggmap)
ak_map <- get_map("tanana", zoom = 4)
ggmap(ak_map) + geom_point(data = ak_fire, aes(x = Longitude, y = Latitude),
                           color = "red", alpha = 0.2, size = 0.5) + 
  facet_wrap(~ GenCause, ncol = 3)

# Just for 2000-2016
ggmap(ak_map) + geom_point(data = subset(ak_fire, FIRESEASON %in% c(2000:2016)),
...
ggmap(ak_map) + geom_point(data = ak_fire, aes(x = Longitude, y = Latitude),
                           color = "red", alpha = 0.2, size = 0.5) + 
  facet_wrap(~ PrimFuel, ncol = 4)

ak_fire <- dplyr::mutate(ak_fire, 
                        fireseason = as.numeric(as.character(FIRESEASON)),
                        year_group = cut(as.numeric(as.character(fireseason)),
                                         breaks = c(1939, 1950, 1970, 1990,
                                                    2010, 2016),
                                         labels = c("1939-1949", "1950-1969",
                                                    "1970-1989", "1990-2009",
                                                    "2010-2016"))) %>%
...
=======
```{r}
>>>>>>> 7964c7d323c453e6c9ca3a74d5e18d492a979b37
library(dplyr)
## BA: Directly pulled from http://afsmaps.blm.gov/imf_firehistory/imf.jsp?site=firehistory
lightning_dir <- "~/Downloads/Lightning1986Thru2015"
library(maptools)
ak_light <- readShapeSpatial(paste(lightning_dir,
                                  "TOALightning2012Thru2015", sep = "/"))
ak_light <- ak_light@data
summary(ak_light$LATITUDE)
summary(ak_light$LONGITUDE)

ggplot(ak_light, aes(x = LONGITUDE, y = LATITUDE)) + geom_point()
```

-RG: I found a conversion formula to convert albers projeciton lat longs to a more useable lat long. However it's in Java. Perhaps someone wants to write the r code converstion?
...
ext <- extent(-180, -130, 50, 75)
  # ext <- drawExtent(show=TRUE) # Or you could use this to draw the bounding box manually (click on opposite corners of the rectangle you want)
...
Remove these `gimms` files: 

```{r}
file.remove(grep("geo13", list.files(), value = TRUE))
```
