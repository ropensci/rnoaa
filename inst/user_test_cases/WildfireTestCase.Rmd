...
```{r}
options("noaakey" = Sys.getenv("noaakey"))
alaska_stations <- ncdc_stations(datasetid = "GHCND", locationid = "FIPS:02", 
                                 limit = 1000)
names <- alaska_stations$data$name
ids <- alaska_stations$data$id
lat <- alaska_stations$data$latitude
lon <- alaska_stations$data$longitude
ak_station_names <- data_frame(names, ids, lat, lon)
ak_station_names$ids <- gsub("GHCND:", "", ak_station_names$ids)
alaska_weather <- meteo_pull_monitors(ak_station_names$ids, 
                                      date_min = "2000-01-01", 
