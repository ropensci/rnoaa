...
author: Brooke Anderson (BA), Rachel Severson (RS)
...
BA: It may work, for daily wind speeds, to use the Global Historical Climatology Network Daily data. If you have monitor ids, you can use the `meteo_pull_monitors` function from `rnoaa` to pull data from this source. If you have the FIPS code for a county, you can use the function ... in `countyweather` (a package Rachel is developing) to get the station IDs for all relevant stations in a county, and the ... function from that same package pulls and aggregates all data from GHCND stations in the county. 
